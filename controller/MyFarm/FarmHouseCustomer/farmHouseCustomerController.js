...
import farmHouseCustomerModel from "../../../model/MyFarm/FarmHouseCustomer/farmHouseCustomerModel.js";


dotenv.config();

export const farmCustomerRegister = async (req, res) => {
  try {
    ...
    const user = await farmHouseCustomerModel.findOne({
      where: { mobileNumber },
      raw: true,
      paranoid: false,
    });
...
export const verifyOtpForCustomer = async (req, res) => {
  try {
    ...
    const newUser = await farmHouseCustomerModel.create({
      name: decoded.name,
...
export const farmCustomerLogin = async (req, res) => {
  try {
